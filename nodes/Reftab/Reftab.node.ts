...
async function makeReftabRequest(
	...
	try {
		...
		if (fullResponse.statusCode >= 400) {
			...
			if (typeof body === 'string') {
				try {
					...
					errorMessage = parsed.message || parsed.error || parsed.msg || parsed.reason || body;
				} catch {
					errorMessage = body;
				}
			} else if (typeof body === 'object' && body !== null) {
				const bodyObj = body as Record<string, unknown>;
				errorMessage = String(bodyObj.message || bodyObj.error || bodyObj.msg || bodyObj.reason || JSON.stringify(body));
			}
		...
		if (error instanceof Error && error.message.startsWith('Reftab API Error')) {
			throw error;
		}
...
async function makeReftabPutRequest(
	...
	const current = await makeReftabRequest(context, 'GET', endpoint) as IDataObject;
		...
		loadOptions: {
			async getLocations(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				try {
					...
					const flattenLocations = (items: Record<string, unknown>[], prefix = ''): INodePropertyOptions[] => {
						const result: INodePropertyOptions[] = [];
						for (const loc of items) {
							const name = prefix ? `${prefix} > ${loc.name}` : String(loc.name || '');
							result.push({
								name,
							...
							if (Array.isArray(loc.children) && loc.children.length > 0) {
								result.push(...flattenLocations(loc.children as Record<string, unknown>[], name));
							}
						}
						return result;
					};
			...
			async getNextAssetId(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				try {
					const credentials = await this.getCredentials('reftabApi');
					const publicKey = credentials.publicKey as string;
					const secretKey = credentials.secretKey as string;
			...
			try {
				if (resource === 'asset') {
					if (operation === 'get') {
						...
						const additionalParams = this.getNodeParameter('additionalParams', i) as IDataObject;
						const fieldFilters = this.getNodeParameter('fieldFilters', i) as IDataObject;
						...
						if (additionalParams) {
							if (additionalParams.offset !== undefined && additionalParams.offset !== '') {
								queryParams.push(`offset=${additionalParams.offset}`);
							}
							if (additionalParams.clid !== undefined && additionalParams.clid !== '') {
								queryParams.push(`clid=${additionalParams.clid}`);
							}
							if (additionalParams.cid !== undefined && additionalParams.cid !== '') {
								queryParams.push(`cid=${additionalParams.cid}`);
							}
							if (additionalParams.status !== undefined && additionalParams.status !== '') {
								queryParams.push(`status=${additionalParams.status}`);
							}
							if (additionalParams.loan !== undefined && additionalParams.loan !== '') {
								queryParams.push(`loan=${additionalParams.loan}`);
							}
							if (additionalParams.loanee !== undefined && additionalParams.loanee !== '') {
						...
						if (fieldFilters && fieldFilters.filters) {
							const filters = fieldFilters.filters as Array<{ fid: string; value: string }>;
							if (filters.length > 0) {
						...
						const assets = Array.isArray(responseData) ? responseData : [responseData];
						...
						try {
							additionalFields = JSON.parse(additionalFieldsString) as IDataObject;
						} catch {
						...
						const assetId = this.getNodeParameter('assetId', i) as string;
						const assetDataString = this.getNodeParameter('assetData', i) as string;
						...
						returnData.push({
							json: { success: true, id: assetId, ...(responseData as IDataObject) },
						});
					}

				} else if (resource === 'assetMaintenance') {
					if (operation === 'get') {
						const maintenanceId = this.getNodeParameter('maintenanceId', i) as number;
						const responseData = await makeReftabRequest(
							this,
							'GET',
						...
						const limit = this.getNodeParameter('maintenanceLimit', i) as number;
						const additionalParams = this.getNodeParameter('maintenanceAdditionalParams', i) as IDataObject;

						// Build query parameters
						const queryParams: string[] = [`limit=${limit}`];

						if (additionalParams) {
							if (additionalParams.offset !== undefined && additionalParams.offset !== '' && additionalParams.offset !== 0) {
								queryParams.push(`offset=${additionalParams.offset}`);
							}
							if (additionalParams.clid !== undefined && additionalParams.clid !== '') {
								queryParams.push(`clid=${additionalParams.clid}`);
							}
							if (additionalParams.aid !== undefined && additionalParams.aid !== '') {
								queryParams.push(`aid=${encodeURIComponent(String(additionalParams.aid))}`);
							}
							if (additionalParams.amid !== undefined && additionalParams.amid !== '' && additionalParams.amid !== 0) {
								queryParams.push(`amid=${additionalParams.amid}`);
							}
							if (additionalParams.mnid !== undefined && additionalParams.mnid !== '') {
								queryParams.push(`mnid=${encodeURIComponent(String(additionalParams.mnid))}`);
							}
							if (additionalParams.uid !== undefined && additionalParams.uid !== '' && additionalParams.uid !== 0) {
								queryParams.push(`uid=${additionalParams.uid}`);
							}
							// Handle assigned user email lookup
							if (additionalParams.assignedEmail !== undefined && additionalParams.assignedEmail !== '') {
								const userId = await lookupUserByEmail(this, String(additionalParams.assignedEmail));
								queryParams.push(`uid=${userId}`);
							}
							if (additionalParams.completed !== undefined && additionalParams.completed !== '') {
						...
						const maintenanceArray = Array.isArray(maintenances) ? maintenances : [maintenances];
						...
						const assetId = this.getNodeParameter('maintenanceAssetId', i) as string;
						const mnid = this.getNodeParameter('maintenanceMnid', i) as number;
						const startDate = this.getNodeParameter('maintenanceStart', i) as string;
						const dueDate = this.getNodeParameter('maintenanceDue', i) as string;
						const createOptions = this.getNodeParameter('maintenanceCreateOptions', i) as IDataObject;
						...
						if (createOptions) {
							if (createOptions.assignedEmail !== undefined && createOptions.assignedEmail !== '') {
								const userId = await lookupUserByEmail(this, String(createOptions.assignedEmail));
								body.assignedUid = userId;
							} else if (createOptions.assignedUid !== undefined && createOptions.assignedUid !== '' && createOptions.assignedUid !== 0) {
					...
					if (operation === 'get') {
						...
						const additionalParams = this.getNodeParameter('loanAdditionalParams', i) as IDataObject;
						...
						if (additionalParams) {
							...
							if (additionalParams.licid !== undefined && additionalParams.licid !== '' && additionalParams.licid !== 0) {
								queryParams.push(`licid=${additionalParams.licid}`);
							}
							if (additionalParams.kid !== undefined && additionalParams.kid !== '' && additionalParams.kid !== 0) {
								queryParams.push(`kid=${additionalParams.kid}`);
							}
							if (additionalParams.accid !== undefined && additionalParams.accid !== '' && additionalParams.accid !== 0) {
								queryParams.push(`accid=${additionalParams.accid}`);
							}
							if (additionalParams.lid !== undefined && additionalParams.lid !== '' && additionalParams.lid !== 0) {
								queryParams.push(`lid=${additionalParams.lid}`);
							}
							if (additionalParams.loan_uid !== undefined && additionalParams.loan_uid !== '' && additionalParams.loan_uid !== 0) {
								queryParams.push(`loan_uid=${additionalParams.loan_uid}`);
							}
							if (additionalParams.lnid !== undefined && additionalParams.lnid !== '' && additionalParams.lnid !== 0) {
								queryParams.push(`lnid=${additionalParams.lnid}`);
							}
							if (additionalParams.status !== undefined && additionalParams.status !== '') {
								queryParams.push(`status=${additionalParams.status}`);
							}
							if (additionalParams.type !== undefined && additionalParams.type !== '') {
								queryParams.push(`type=${additionalParams.type}`);
							}
							if (additionalParams.loanee !== undefined && additionalParams.loanee !== '') {
								queryParams.push(`loanee=${encodeURIComponent(String(additionalParams.loanee))}`);
							}
							if (additionalParams.lgid !== undefined && additionalParams.lgid !== '' && additionalParams.lgid !== 0) {
								queryParams.push(`lgid=${additionalParams.lgid}`);
							}
							if (additionalParams.activitySince !== undefined && additionalParams.activitySince !== '') {
								queryParams.push(`activitySince=${encodeURIComponent(String(additionalParams.activitySince))}`);
							}
							if (additionalParams.activityUntil !== undefined && additionalParams.activityUntil !== '') {
						...
						const loaneeEmail = this.getNodeParameter('loaneeEmail', i) as string;
						const loanItems = this.getNodeParameter('loanItems', i) as IDataObject;
						const loanOptions = this.getNodeParameter('loanOptions', i) as IDataObject;
						...
						if (loanItems.accessories) {
							const accessories = loanItems.accessories as Array<{ accid: number }>;
						...
						const loanId = this.getNodeParameter('loanId', i) as number;
						const updateFields = this.getNodeParameter('loanUpdateFields', i) as IDataObject;
						...
						const loanId = this.getNodeParameter('loanId', i) as number;
						const checkInLocation = this.getNodeParameter('checkInLocation', i) as string;
						const checkInNotes = this.getNodeParameter('checkInNotes', i) as string;
					...
					if (operation === 'get') {
						const reservationId = this.getNodeParameter('reservationId', i) as number;
						const responseData = await makeReftabRequest(
							this,
							'GET',
						...
						const limit = this.getNodeParameter('reservationLimit', i) as number;
						const additionalParams = this.getNodeParameter('reservationAdditionalParams', i) as IDataObject;
						...
						if (additionalParams) {
							...
							if (additionalParams.loaneeEmail !== undefined && additionalParams.loaneeEmail !== '') {
						...
						const loaneeEmail = this.getNodeParameter('reservationLoaneeEmail', i) as string;
						const startDate = this.getNodeParameter('reservationStart', i) as string;
						const endDate = this.getNodeParameter('reservationEnd', i) as string;
						const reservationItems = this.getNodeParameter('reservationItems', i) as IDataObject;
						const reservationOptions = this.getNodeParameter('reservationOptions', i) as IDataObject;
						...
						if (reservationItems.licenses) {
							const licenses = reservationItems.licenses as Array<{ licid: number }>;
							body.licids = licenses.map(l => l.licid).filter(l => l);
						}
						if (reservationItems.accessories) {
							const accessories = reservationItems.accessories as Array<{ accid: number }>;
							body.accids = accessories.map(a => a.accid).filter(a => a);
						}
						...
						const reservationId = this.getNodeParameter('reservationId', i) as number;
						const updateFields = this.getNodeParameter('reservationUpdateFields', i) as IDataObject;
						...
						const reservationId = this.getNodeParameter('reservationId', i) as number;
						const fulfillOptions = this.getNodeParameter('fulfillOptions', i) as IDataObject;
					...
					const queryParams = this.getNodeParameter('queryParameters', i) as string;
